...
public class STTS implements Payload {
    ...
    private static final String TAG = STTS.class.getSimpleName();
    ...
    public void read(ChannelBuffer in) {
        ...
        if (D) {
        	Log.d(TAG, "no of time to sample records: " + count);
        }
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (D) {
            	Log.d(TAG, "#" + i + " sampleCount: " + record.sampleCount + " sampleDuration: " + record.sampleDuration);
            }
            records.add(record);
        }
    }
