...
public class STSD implements Payload {
    ...
    private static final String TAG = STSD.class.getSimpleName();
    ...
    public void read(ChannelBuffer in) {
        ...
        if (D) {
        	Log.d(TAG, "no of sample descripton records: " + count);
        }
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (D) {
            	Log.d(TAG, "sample description: " + record.type + ", " + record.sampleDescription);
            }
            records.add(record);
        }
    }
