...
public class MVHD implements Payload {
    ...
    private static final String TAG = MVHD.class.getSimpleName();
    ...
    public void read(ChannelBuffer in) {
        version = in.readByte();
        if (D) {
        	Log.d(TAG, "version: " + Utils.toHex(version));
        }
        flags = new byte[3];
        in.readBytes(flags);
        if (version == 0x00) {
        ...
        volume = in.readShort();
        if (D) {
        	Log.d(TAG, "creationTime " + creationTime + " modificationTime " + modificationTime + " timeScale " + timeScale + " duration " + duration + " playbackRate " + playbackRate + " volume " + volume);
        }
        ...
        for (int i = 0; i < transformMatrix.length; i++) {
            ...
            if (D) {
            	Log.d(TAG, "transform matrix[" + i + "]: " + transformMatrix[i]);
            }
        }
        reserved3 = new int[6];
