...
public class FlvReader implements RtmpReader {
    ...
    private static final String TAG = FlvReader.class.getSimpleName();
    ...
    public FlvReader(final String path) {
        ...
        if (D) {
        	Log.d(TAG, "flv file metadata: " + metadata);
        }
    }
    ...
    public long seek(final long time) {
        if (D) {
        	Log.d(TAG, "trying to seek to: " + time);
        }
        ...
        try {
            ...
            while(hasPrev()) {
                ...
                if(cursor.getHeader().isVideo() && isSyncFrame(cursor)) {
                    if (D) {
                    	Log.d(TAG, "returned seek frame / position: " + cursor);
                    }
