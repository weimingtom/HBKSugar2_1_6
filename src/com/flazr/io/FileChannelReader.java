...
public class FileChannelReader implements BufferReader {
    ...
    private static final String TAG = FileChannelReader.class.getSimpleName();
    ...
    public FileChannelReader(final File file) {
        ...
        if (D) {
        	Log.i(TAG, "opened file: " + absolutePath);
        }
    }
    ...
    public void close() {
        try {
        	...
        	if (D) {
        		Log.w(TAG, "error closing file " + absolutePath + ": " + e.getMessage());
        	}
        }
        if (D) {
