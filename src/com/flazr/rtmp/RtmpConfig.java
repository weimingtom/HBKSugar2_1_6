...
public class RtmpConfig {
    ...
    private static final String TAG = RtmpConfig.class.getSimpleName();
    ...
    private static void configure(Type type) {
        ...
        if(!propsFile.exists()) {
        	if (D) {
        		Log.w(TAG, "" + propsFile.getAbsolutePath() + " not found, will use configuration defaults");
        	}
        } else {
        	if (D) {
        		Log.i(TAG, "loading config from: " + propsFile.getAbsolutePath());
        	}
            ...
            switch(type) {
                ...
                case SERVER_STOP:
                    ...
                    if(!homeFile.exists()) {
                    	if (D) {
                    		Log.e(TAG, "home dir does not exist, aborting: " + homeFile.getAbsolutePath());
                    	}
                    	throw new RuntimeException("home dir does not exist: " + homeFile.getAbsolutePath());
                    }
                    if (D) {
                    	Log.i(TAG, "home dir: '" + homeFile.getAbsolutePath() + "'");
                    	Log.i(TAG, "server port: " + SERVER_PORT + " (stop " + SERVER_STOP_PORT + ")");
                    }
                    break;
                case PROXY:
                case PROXY_STOP:
                    ...
                    if (D) {
                    	Log.i(TAG, "proxy port: " + PROXY_PORT + " (stop " + PROXY_STOP_PORT + ")");
                    	Log.i(TAG, "proxy remote host: " + PROXY_REMOTE_HOST + " port: " + PROXY_REMOTE_PORT);
                    }
                    break;
            }
        }        
    ...
    private static Integer parseInt(String s) {
        try {
        	...
        	if (D) {
        		Log.w(TAG, "unable to parse into integer value: " + e.getMessage());
        	}
        	return null;
        }
    }
