...
public class ClientHandler extends SimpleChannelUpstreamHandler {
    ...
    public void setSwfvBytes(byte[] swfvBytes) {
        ...
        if (D) {
        	Log.i(TAG, "set swf verification bytes: " + Utils.toHex(swfvBytes));
        }
    }

    public ClientHandler(ClientOptions options, Handler handler, int timerTimeout) {
    ...
    private void writeCommandExpectingResult(Channel channel, Command command) {
        ...
        if (D) {
        	Log.i(TAG, "sending command (expecting result): " + command);
        }
    ...
    public void channelConnected(ChannelHandlerContext ctx, ChannelStateEvent e) {
        if (D) {
        	Log.i(TAG, "handshake complete, sending 'connect'");
        }
        ...
        switch(message.getHeader().getMessageType()) {
            ...
            case CONTROL:
                ...
                switch(control.getType()) {
                    case PING_REQUEST:
                        ...
                        if (D) {
                        	Log.d(TAG, "server ping: " + time);
                        }
                        Control pong = Control.pingResponse(time);
                        if (D) {
                        	Log.d(TAG, "sending ping response: " + pong);
                        }
                        channel.write(pong);
                        break;
                    case SWFV_REQUEST:
                    ...
                    case STREAM_BEGIN:
                        if(publisher != null) {
                        	if (D) {
                        		Log.i(TAG, "publish mode, stream begin, will start " + control);
                        	}
                    ...
                    default:
                    	if (D) {
                    		Log.d(TAG, "ignoring control message: " + control);
                    	}
                }
                break;
            case METADATA_AMF0:
            case METADATA_AMF3:
                ...
                if(metadata.getName().equals("onMetaData")) {
                	if (D) {
                		Log.d(TAG, "writing 'onMetaData': " + metadata);
                	}
                    if (this.handler != null) {
                    	this.duration = metadata.getDuration();
                    	handler.sendMessage(handler.obtainMessage(
                    			DownloadThread.MESSAGE_DURATION_THREAD, 
                    			Double.valueOf(duration)));
                    }
                	writer.write(message);
                } else {
                	if (D) {
                		Log.d(TAG, "ignoring metadata: " + metadata);
                	}
                }
                break;
            case AUDIO:
            ...
            case AGGREGATE:                
                ...
                if((bytesRead - bytesReadLastSent) > bytesReadWindow) {
                	if (D) {
                		Log.d(TAG, "sending bytes read ack " + bytesRead);
                	}
            ...
            case COMMAND_AMF3:
                ...
                if(name.equals("_result")) {
                    ...
                    if(resultFor.equals("connect")) {
                        ...
                        if (D) {
                        	Log.d(TAG, "streamId to use: " + streamId);
                        }
                        if(options.getPublishType() != null) { // TODO append, record                            
                            ...
                            writer = new FlvWriter(options.getStart(), options.getSaveAs(), this.handler, this.timerTimeout);
                            channel.write(Command.play(streamId, options));
                            channel.write(Control.setBuffer(streamId, 0));
                        }
                    } else {
                    	if (D) {
                    		Log.w(TAG, "un-handled server result for: " + resultFor);
                    	}
                    }
                    ...
                    if (code.equals("NetStream.Failed") // TODO cleanup
                    	...
                    	if (D) {
                    		Log.i(TAG, "disconnecting, code: " + code + ", bytes read: " + bytesRead);
                    	}
                    	channel.close();
                        return;
                    }
                    ...
                    if (publisher != null && code.equals("NetStream.Unpublish.Success")) {
                        if (D) {
                        	Log.i(TAG, "unpublish success, closing channel");
                        }
                	...
                	if (D) {
                		Log.i(TAG, "server called close, closing channel");
                	}
                	channel.close();
                    return;
                } else if(name.equals("_error")) {
                	if (D) {
                		Log.e(TAG, "closing channel, server resonded with error: " + command);
                	}
                	channel.close();
                    return;
                } else {
                	if (D) {
                    	Log.w(TAG, "ignoring server command: " + command);
                	}
                }
                break;
            case BYTES_READ:
            	if (D) {
            		Log.i(TAG, "ack from server: " + message);
            	}
            	break;
            ...
            default:
            	if (D) {
            		Log.i(TAG, "ignoring rtmp message: " + message);
            	}
        }
