...
public class ClientOptions {
    ...
    private static final String TAG = ClientOptions.class.getSimpleName();
	...
	*/
    ...
    public void parseUrl(String url) {
        ...
        if (portString == null) {
        	if (D) {
        		Log.d(TAG, "port is null in url, will use default 1935");
        	}
        } else {
            portString = portString.substring(1); // skip the ':'
            if (D) {
            	Log.d(TAG, "port = '" + portString + "'");
            }
        }
        ...
        if (D) {
        	Log.d(TAG, "playName = '" + streamName + "'");
        }
        rtmpe = protocol.equalsIgnoreCase("rtmpe");
        if(rtmpe) {
        	if (D) {
        		Log.d(TAG, "rtmpe requested, will use encryption");
        	}
        }        
    }
    ...
    public boolean parseCli(final String[] args, int startPos) {
    	/*
        ...
        */
    	start = startPos;
        String[] actualArgs = args; //line.getArgs();
    ...
    public void initSwfVerification(File localSwfFile) {
        if (D) {
        	Log.i(TAG, "initializing swf verification data for: " + localSwfFile.getAbsolutePath());
        }
        ...
        swfHash = hash;
        if (D) {
        	Log.i(TAG, "swf verification initialized - size: " + swfSize + ", hash: " + Utils.toHex(swfHash));
        }
    }
