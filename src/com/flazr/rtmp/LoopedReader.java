...
public class LoopedReader implements RtmpReader {
    ...
    private static final String TAG = LoopedReader.class.getSimpleName();
    ...
    public LoopedReader(final RtmpReader reader, final int loopCount) {
        ...
        if (D) {
        	Log.i(TAG, "looped reader init: count " + loopCount);
        }
    }
    ...
    public boolean hasNext() {
        ...
        if(loopsCompleted < loopCount) {
            reader.seek(0);
            if (D) {
            	Log.i(TAG, "re-wound media after loop #" + loopsCompleted);
            }
            return true;
        }
        return false;
