...
public class RtmpHandshake {
    ...
    private static final String TAG = RtmpHandshake.class.getSimpleName();
    ...
    private void initCiphers() {
        ...
        try {
            ...
            if (D) {
            	Log.i(TAG, "initialized encryption / decryption ciphers");
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    ...
    public ChannelBuffer encodeClient1() {        
        ...
        if (D) {
        	Log.i(TAG, "using client version " + Utils.toHex(clientVersionToUse));
        }
        if (validationType == 0) {
        ...
        if (D) {
        	Log.d(TAG, "creating client part 1, validation type: " + validationType);
        }
        initKeyPair();
    ...
    private void decodeServer0(ChannelBuffer in) {
        ...
        if(rtmpe &&  flag != 0x06) {
        	if (D) {
        		Log.w(TAG, "server does not support rtmpe! falling back to rtmp");
        	}
        	rtmpe = false;
        }
    }
    ...
    private void decodeServer1(ChannelBuffer in) {
        ...
        if (D) {
        	Log.d(TAG, "server time: " + Utils.toHex(peerTime) + ", version: " + Utils.toHex(serverVersion));
        }
        if(swfHash != null) {
            // swf verification
            ...
            if (D) {
            	Log.i(TAG, "calculated swf verification response: " + Utils.toHex(swfvBytes));
            }
        }
        if(validationType == 0) {
        ...
        if (D) {
        	Log.d(TAG, "processing server part 1, validation type: " + validationType);
        }
        ...
        if (!Arrays.equals(peerPartOneDigest, expected)) {            
            ...
            if (D) {
            	Log.w(TAG, "server part 1 validation failed for type " + validationType + ", will try with type " + altValidationType);
            }
        ...
        if (D) {
        	Log.i(TAG, "server part 1 validation success");
        }
    ...
    private void decodeServer2(ChannelBuffer in) {
        ...
        if (D) {
        	Log.d(TAG, "processing server part 2 for validation");
        }
        ...
        if (D) {
        	Log.i(TAG, "server part 2 validation success");                   
        }
    }
    ...
    public ChannelBuffer encodeClient2() {
        ...
        if (D) {
        	Log.d(TAG, "creating client part 2 for validation");
        }
        ChannelBuffer out = generateRandomHandshake();
    ...
    public void decodeClient0(ChannelBuffer in) {
        ...
        if (D) {
        	Log.d(TAG, "client first byte " + Utils.toHex(firstByte) + ", rtmpe: " + rtmpe);  
        }
    }
    ...
    public boolean decodeClient1(ChannelBuffer in) {
        ...
        if (D) {
        	Log.d(TAG, "client time: " + Utils.toHex(peerTime) + ", version: " + Utils.toHex(clientVersion));
        }
        ...
        if (D) {
        	Log.d(TAG, "processing client part 1 for validation type: " + validationType);
        }
        initKeyPair();
        int digestOffset = digestOffset(in, validationType);
        peerPartOneDigest = new byte[DIGEST_SIZE];
        ...
        if (D) {
        	Log.i(TAG, "client part 1 validation success");
        }
        int publicKeyOffset = publicKeyOffset(in, validationType);
        peerPublicKey = new byte[PUBLIC_KEY_SIZE];
        in.getBytes(publicKeyOffset, peerPublicKey);
    ...
    public ChannelBuffer encodeServer1() {        
        ...
        if (D) {
        	Log.d(TAG, "creating server part 1 for validation type: " + validationType);
        }
        int publicKeyOffset = publicKeyOffset(out, validationType);
        out.setBytes(publicKeyOffset, ownPublicKey);
        int digestOffset = digestOffset(out, validationType);
    ...
    public void decodeClient2(ChannelBuffer raw) {
        ...
        if (D) {
        	Log.d(TAG, "processing client part 2 for validation");
        }
        ...
        if (D) {
        	Log.i(TAG, "client part 2 validation success");                
        }
    }
    ...
    public ChannelBuffer encodeServer2() {
        ...
        if (D) {
        	Log.d(TAG, "creating server part 2 for validation");
        }
        ChannelBuffer out = generateRandomHandshake();
