...
public class ProxyHandler extends SimpleChannelUpstreamHandler {
    ...
    public void channelOpen(ChannelHandlerContext ctx, ChannelStateEvent e) {        
        ...
        f.addListener(new ChannelFutureListener() {
            @Override public void operationComplete(ChannelFuture future) throws Exception {
                if (future.isSuccess()) {
                    if (D) {
                    	Log.i(TAG, "connected to remote host: " + remoteHost + ", port: " + remotePort);
                    }
    ...
    public void channelClosed(ChannelHandlerContext ctx, ChannelStateEvent e) {
        if (D) {
        	Log.i(TAG, "closing inbound channel");
        }
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, ExceptionEvent e) {
        if (D) {
        	Log.i(TAG, "inbound exception: " + e.getCause().getMessage());
        }
    ...
    private class OutboundHandler extends SimpleChannelUpstreamHandler {
        ...
        public OutboundHandler(Channel inboundChannel) {
        	if (D) {
        		Log.i(TAG, "opening outbound channel");
        	}
        ...
        public void channelClosed(ChannelHandlerContext ctx, ChannelStateEvent e) {
            if (D) {
            	Log.i(TAG, "closing outbound channel");
            }
        ...
        public void exceptionCaught(ChannelHandlerContext ctx, ExceptionEvent e) {
            if (D) {
            	Log.i(TAG, "outbound exception: " + e.getCause().getMessage());
            }
            closeOnFlush(e.getChannel());
        }
    }
