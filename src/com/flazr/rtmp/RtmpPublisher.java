...
public abstract class RtmpPublisher {
    ...
    public RtmpPublisher(final RtmpReader reader, final int streamId, final int bufferDuration, boolean useSharedTimer) {
        ...
        if (D) {
        	Log.d(TAG, "publisher init, streamId: " + streamId);
        }
    }
    ...
    public boolean handle(final MessageEvent me) {        
        if(me.getMessage() instanceof Event) {
            ...
            if(pe.conversationId != currentConversationId) {
            	if (D) {
            		Log.d(TAG, "stopping obsolete conversation id: " + pe.getConversationId() + ", current: " + currentConversationId);
            	}
            	return true;
            }
            write(me.getChannel());
    ...
    public void start(final Channel channel, final int seekTimeRequested, final RtmpMessage ... messages) {
        ...
        if (D) {
        	Log.d(TAG, "play start, seek requested: " + seekTimeRequested + " actual seek: " + seekTime + ", play length: " + playLength + ", conversation: " + currentConversationId);
        }
    ...
    public synchronized void write(final Channel channel) { // only place using :SYNCHRONIZED
        ...
        if(D) {
            Log.d(TAG, "elapsed: " + elapsedTimePlusSeek + ", streamed: " + timePosition + ", buffer: " + clientBuffer + ", factor: " + compensationFactor + ", delay: " + delay);
        }
        ...
        future.addListener(new ChannelFutureListener() {
            @Override public void operationComplete(final ChannelFuture cf) {
                ...
                if(completedIn > 2000) {
                	if (D) {
                		Log.w(TAG, "channel busy? time taken to write last message: " + completedIn);
                	}
                }                
    ...
    private void stop(final Channel channel) {
        ...
        final long elapsedTime = System.currentTimeMillis() - startTime;
        if (D) {
        	Log.i(TAG, "finished, start: " + (seekTime / 1000) + ", elapsed " + (elapsedTime / 1000) + ", streamed: " + ((timePosition - seekTime) / 1000));
        }
