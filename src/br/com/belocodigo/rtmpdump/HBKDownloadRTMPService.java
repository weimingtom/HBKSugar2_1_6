...
public class HBKDownloadRTMPService extends Service {
		...
		public void handleMessage(Message msg) {
			...
			case RTMPDownloadThread.MESSAGE_CLOSE_THREAD:
				if (getIsRetry() && 
					...
					//isShowLog = false;
					updateUI(true);
					downloadThread = new RTMPDownloadThread(msg.arg1, url, mp3filename, infoText, tabUrl, detailUrl, downloadHandler, 1);
					downloadThread.setResume(true);
    ...
    private void startDownload(int startId) {
		...
		retryNum = 0;
	    downloadThread = new RTMPDownloadThread(startId, url, mp3filename, infoText, tabUrl, detailUrl, downloadHandler, 1);
		downloadThread.start();
    }
    ...
    private boolean getIsRetry() {
    	return ENABLE_RETRY && HBKDownloadActivity.getLastEnableRetry(this);
    }
}
